...
<div class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    <h1 class="h4 mb-0 d-flex align-items-center gap-2">Aulas · <?= htmlspecialchars((string)($course['title'] ?? '')) ?></h1>
    <div class="d-flex gap-2">
      <a href="/secure/adm/aluno/bunny" class="btn btn-outline-secondary btn-sm">Bunny Tools</a>
      <a href="/secure/adm/aluno/lessons/create?course_id=<?= (int)$course['id'] ?>" class="btn btn-primary btn-sm">Nova Aula</a>
              ...
              <td style="width:72px;">
                <div class="btn-group btn-group-sm" role="group">
                  <form method="post" action="/secure/adm/aluno/lessons/reorder">
                    ...
                    <button class="btn btn-outline-secondary" title="Acima" type="submit"></button>
                  </form>
                  <form method="post" action="/secure/adm/aluno/lessons/reorder">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>"/>
                    <input type="hidden" name="id" value="<?= (int)$L['id'] ?>"/>
                    <input type="hidden" name="dir" value="down"/>
                    <button class="btn btn-outline-secondary" title="Abaixo" type="submit"></button>
                  </form>
                </div>
              </td>
              ...
              <td class="text-end">
                <div class="btn-group btn-group-sm" role="group">
                  <form method="post" action="/secure/adm/aluno/lessons/toggle">
                    ...
                    <button class="btn btn-outline-secondary" type="submit"><?= !empty($L['is_enabled']) ? 'Desativar' : 'Ativar' ?></button>
                  </form>
                  <form method="post" action="/secure/adm/aluno/lessons/set-preview">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>"/>
                    <input type="hidden" name="id" value="<?= (int)$L['id'] ?>"/>
                    <input type="hidden" name="course_id" value="<?= (int)$course['id'] ?>"/>
                    <button class="btn btn-outline-success" type="submit">Prévia</button>
                  </form>
                  <a href="/secure/adm/aluno/lessons/edit?id=<?= (int)$L['id'] ?>" class="btn btn-outline-secondary"></a>
