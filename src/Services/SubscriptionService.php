...
class SubscriptionService
{
    ...
    /**
     * Lista todos os planos ativos com dados de front display para o terminal
     */
    public function getActivePlans(): array
    {
        ...
        // Enrich with front display data for terminal system
        try {
            $frontDisplay = Database::fetchAll(
                "SELECT * FROM plan_front_display WHERE system_key = 'terminal' AND is_visible = TRUE"
            );
            $fdMap = [];
            foreach ($frontDisplay as $fd) {
                $fdMap[$fd['plan_id']] = $fd;
            }
            
            foreach ($plans as &$plan) {
                if (isset($fdMap[$plan['id']])) {
                    $fd = $fdMap[$plan['id']];
                    // Override with front display data if available
                    if (!empty($fd['display_name'])) $plan['display_name'] = $fd['display_name'];
                    if (!empty($fd['price_display'])) $plan['price_display'] = $fd['price_display'];
                    if (!empty($fd['price_subtitle'])) $plan['price_subtitle'] = $fd['price_subtitle'];
                    if (!empty($fd['description'])) $plan['description'] = $fd['description'];
                    if (!empty($fd['features'])) $plan['features'] = $fd['features'];
                    if (!empty($fd['cta_label'])) $plan['cta_label'] = $fd['cta_label'];
                    if (!empty($fd['cta_url'])) $plan['cta_url'] = $fd['cta_url'];
                    $plan['is_highlighted'] = !empty($fd['is_highlighted']);
                    if (!empty($fd['badge_text'])) $plan['badge_text'] = $fd['badge_text'];
                }
            }
            unset($plan);
        } catch (\Throwable $t) {
            // Table may not exist yet, continue with base plan data
        }
