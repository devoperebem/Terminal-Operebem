...
class AdminSecureController extends BaseController
{
    ...
    /**
     * PIN gate — exibe formulário de PIN de 6 dígitos
     * GET /secure/adm/pin
     */
    public function pinForm(): void
    {
        $pin = trim((string)($_ENV['ADMIN_PIN'] ?? ''));
        if ($pin === '') {
            $this->view('admin_secure/pin', [ 'title' => 'Secure Admin - PIN', 'error' => 'missing', 'footerVariant' => 'admin-login' ]);
            return;
        }
        $this->view('admin_secure/pin', [ 'title' => 'Secure Admin - PIN', 'error' => $_GET['error'] ?? '', 'footerVariant' => 'admin-login' ]);
    }

    /**
     * Verifica PIN de segurança
     * POST /secure/adm/pin
     */
    public function verifyPin(): void
    {
        if (!$this->validateCsrf()) { $this->redirect('/secure/adm/pin?error=csrf'); }

        $expectedPin = trim((string)($_ENV['ADMIN_PIN'] ?? ''));
        if ($expectedPin === '') { $this->redirect('/secure/adm/pin?error=missing'); }
        ...
        try {
            ...
            $count = (int)(Database::fetch(
                'SELECT COUNT(*) AS c FROM login_attempts WHERE email = ? AND attempted_at > ? AND success = 0',
                ['admin-pin:' . $ip, $since]
            )['c'] ?? 0);
            if ($count >= 5) { $this->redirect('/secure/adm/pin?error=ratelimit'); }
        } catch (\Throwable $t) { /* ignore */ }

        $submittedPin = trim((string)($_POST['pin'] ?? ''));

        if (!hash_equals($expectedPin, $submittedPin)) {
            try {
            ...
            try { Application::getInstance()->logger()->warning('Admin PIN invalid', ['ip' => $ip]); } catch (\Throwable $t) {}
            $this->redirect('/secure/adm/pin?error=invalid');
        }

        // PIN correto — gravar na sessão
        if (session_status() === PHP_SESSION_ACTIVE) session_regenerate_id(true);
        $_SESSION['admin_pin_verified'] = true;
        $_SESSION['admin_pin_verified_at'] = time();
        ...
        try { Application::getInstance()->logger()->info('Admin PIN verified', ['ip' => $ip]); } catch (\Throwable $t) {}

        $this->redirect('/secure/adm/login');
    }

    /**
     * Verifica se o PIN foi validado na sessão (expira em 30 min)
     */
    private function isPinVerified(): bool
    {
        $pin = trim((string)($_ENV['ADMIN_PIN'] ?? ''));
        if ($pin === '') return true; // PIN não configurado — bypass
        if (empty($_SESSION['admin_pin_verified'])) return false;
        $verifiedAt = (int)($_SESSION['admin_pin_verified_at'] ?? 0);
        if (time() - $verifiedAt > 1800) { // 30 min
            unset($_SESSION['admin_pin_verified'], $_SESSION['admin_pin_verified_at']);
            return false;
        }
        return true;
    }
    ...
    {
        // Exigir PIN antes de exibir login
        if (!$this->isPinVerified()) {
            $this->redirect('/secure/adm/pin');
            return;
        }
    ...
    public function index(): void
    {
        ...
        try { $since30d = date('Y-m-d H:i:s', time() - 30*24*3600); $r = Database::fetch('SELECT COUNT(*) AS c FROM users WHERE (last_login_at IS NULL OR last_login_at < ?) AND deleted_at IS NULL', [$since30d]); $userStats['inactive_30d'] = (int)($r['c'] ?? 0); } catch (\Throwable $t) {}
        try { $r = Database::fetch('SELECT COUNT(*) AS c FROM users WHERE last_login_at IS NULL AND deleted_at IS NULL'); $userStats['never_logged'] = (int)($r['c'] ?? 0); } catch (\Throwable $t) {}
        ...
        $planDistribution = [];
        try { $planDistribution = Database::fetchAll("SELECT COALESCE(tier, 'FREE') AS tier, COUNT(*) AS total FROM users WHERE deleted_at IS NULL GROUP BY tier ORDER BY total DESC"); } catch (\Throwable $t) {}
    ...
    public function crm(): void
    {
        ...
        try {
            ...
            ");
            // Ensure user_id column exists (may be missing in older tables)
            Database::query("ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE SET NULL");
            ...
            error_log('[CRM] Table setup error: ' . $t->getMessage());
        }
        
        // Sync existing users as leads (converted status) - run every time to catch new users
        try {
            Database::query("
                INSERT INTO crm_leads (user_id, name, email, phone, source, status, created_at, updated_at)
                SELECT 
                    u.id,
                    u.name,
                    u.email,
                    ...
                    CURRENT_TIMESTAMP
                FROM users u
                WHERE NOT EXISTS (SELECT 1 FROM crm_leads WHERE email = u.email)
            ");
        } catch (\Throwable $t) { 
            error_log('[CRM] Sync error: ' . $t->getMessage());
        }
    ...
    public function viewLead(): void
    {
        ...
        try {
            $lead = Database::fetch('SELECT * FROM crm_leads WHERE id = ?', [$id]);
            if (!$lead) {
            ...
            if (!empty($lead['user_id'])) {
                $linkedUser = Database::fetch('SELECT id, name, email, tier, created_at FROM users WHERE id = ?', [$lead['user_id']]);
            }

            // Get interaction history
            $interactions = [];
            try {
                $interactions = Database::fetchAll('SELECT * FROM crm_lead_interactions WHERE lead_id = ? ORDER BY created_at DESC LIMIT 50', [$id]);
            } catch (\Throwable $t) { /* table may not exist */ }

            $this->view('admin_secure/crm_view', [
                'title' => 'Lead #' . $id,
                'footerVariant' => 'admin-auth',
                'lead' => $lead,
            ...
            $_SESSION['flash_error'] = 'Erro ao carregar lead: ' . $e->getMessage();
            header('Location: /secure/adm/crm');
            exit;
        }
    }
    ...
    public function updateLead(): void
    {
        ...
        if (!$id) {
            $_SESSION['flash_error'] = 'ID do lead inválido';
            header('Location: /secure/adm/crm');
            exit;
        }
        ...
        try {
            ...
            $oldLead = Database::fetch('SELECT status FROM crm_leads WHERE id = ?', [$id]);
            $oldStatus = $oldLead['status'] ?? 'unknown';
            ...
            // Log status change
            if ($oldStatus !== $status) {
                Database::insert('crm_lead_interactions', [
                    'lead_id' => $id,
                    'type' => 'status_change',
        ...
        header('Location: /secure/adm/crm/view?id=' . $id);
        exit;
    }

    public function addLeadInteraction(): void
    {
        if (!$this->validateCsrf()) {
            $_SESSION['flash_error'] = 'Token CSRF inválido';
            header('Location: /secure/adm/crm');
        ...
        try {
            ...
            $_SESSION['flash_success'] = 'Interação registrada!';
        } catch (\Throwable $e) {
            $_SESSION['flash_error'] = 'Erro: ' . $e->getMessage();
        }

        header('Location: /secure/adm/crm/view?id=' . $leadId);
        exit;
    }
